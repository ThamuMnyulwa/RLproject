...
if __name__ == '__main__':
  ...
  plt.plot(c_1, label='Realistic, $\epsilon$-greedy $\epsilon = 0.1$',color="blue",lw=2)
  plt.plot(oiv, label='Optimistic Initial value, greedy',color="red",lw=2)
  ...
  plt.yticks(fontsize=12,c='black')
  plt.plot(c_1, label='Realistic, $\epsilon$-greedy $\epsilon = 0.1$',color="blue",lw=2)
  plt.plot(oiv, label='Optimistic, greedy ($\epsilon = 0$)',color="red",lw=2)
  plt.xlabel("Steps", fontsize=15)
  plt.ylabel("Average Reward", fontsize=15)
  plt.legend(loc="best", fontsize=15)
  plt.show()
